...
def fit_func(Vg, m_sqrt, Vt):
    Isd_sqrt=np.sqrt(W*C/(2*L))*m_sqrt*(Vg-Vt)
    return Isd_sqrt
...
for each_name in csv_name:
    ...
    Vg = paramV[:51]
    Isd_sqrt = np.sqrt(paramI[:51])

    popt, pcov = curve_fit(fit_func, Vg, Isd_sqrt, maxfev=100000)
    m_sqrt,Vt = popt[0:2]
    spvals=fit_func(Vg, m_sqrt, Vt)
    plt.plot(Vg, Isd_sqrt, '*')
    plt.plot(Vg, spvals, 'r')
    plt.title(each_name[:-4]+' a=%f, b=%f'%(m_sqrt**2,Vt))
    plt.show()
